    ...
    account_inherit.include({
        ...
        custom_events: {
            'value_changed': function(ev) {
                 ...
                 self.report_options.v_region_reports = ev.data.v_region_reports;
        ...
        render_searchview_buttons: function() {
            ...
            window.catha = this;
            if (this.report_options.v_region_report) {
            if (!this.VregioonesFilters) {
                var fields = {};
                fields['v_region_reports'] = {
                    label: _t('Regiones'),
                    modelName: 'tf.regiones',
                    value: this.report_options.v_region_reports.map(Number),
                };
                this.VregioonesFilters = new M2MFilters(this, fields);
                this.VregioonesFilters.appendTo(this.$searchview_buttons.find('.js_v_region_report_m2m'));
            } else {
                this.$searchview_buttons.find('.js_v_region_report_m2m').append(this.VregioonesFilters.$el);
            }
        }
