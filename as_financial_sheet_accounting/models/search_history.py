...
class AccountAgedReceivable(models.AbstractModel):
    ...
    def _set_context(self, options):
        ctx = super(AccountAgedReceivable, self)._set_context(options)
        ctx.update({
            'v_cost_centers': options['v_cost_centers'],
            'v_cost_center': options['v_cost_center'],
            'v_departments': options['v_departments'],
            'v_department': options['v_department'],
            'v_region_reports': options['v_region_reports'],
            'v_region_report': options['v_region_report'],
        })
        return ctx

class AsReportAgedPartnerBalance(models.AbstractModel):

    _inherit = 'report.account.report_agedpartnerbalance'
    _description = 'Aged Partner Balance Report Heredado'
    ...
    def _get_partner_move_lines(self, account_type, date_from, target_move, period_length):
        ...
        if 'v_cost_center' in ctx:
            if ctx['v_cost_centers'] != []:
                region_centro_departament = """AND (l.cost_center_id IN """+str(ctx['v_cost_centers']).replace('[','(').replace(']',')')+""")"""
        if 'v_department' in ctx:
            if ctx['v_departments'] != []:
                region_centro_departament = """AND (l.department_id IN """+str(ctx['v_departments']).replace('[','(').replace(']',')')+""")"""
        if 'v_region_report' in ctx:
            if ctx['v_region_reports'] != []:
                region_centro_departament = """AND (l.regiones_id IN """+str(ctx['v_region_reports']).replace('[','(').replace(']',')')+""")"""
