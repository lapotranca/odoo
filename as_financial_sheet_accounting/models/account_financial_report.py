...
class ReportAccountFinancialReport(models.Model):
    ...
    v_region_report = fields.Boolean('Allow Region Filters')
    ...
    filter_v_region_reports = []
    filter_v_region_report = True
    ...
    def _with_correct_filters(self):
        ...
        if res.v_region_report:
            res.filter_v_region_reports = []
            res.filter_v_region_report = True
        if res.v_department:
    ...
    @api.model
    def _init_filter_v_region_report(self, options, previous_options=None):
        if not self.filter_v_region_report:
            return

        options['v_region_report'] = self.filter_v_region_report
        CostCenter = self.env['tf.regiones'].sudo()
        options['v_region_reports'] = previous_options and previous_options.get('v_region_reports') or []
        record_ids = [int(acc_grp) for acc_grp in options['v_region_reports']]
        selected_v_region_reports = record_ids and CostCenter.browse(record_ids) or CostCenter
        options['selected_v_region_report_names'] = selected_v_region_reports.mapped('name')

    @api.model
    def _get_options_v_region_report_domain(self, options):
        domain = []
        if options.get('v_region_reports'):
            v_region_report_ids = [int(acc_grp) for acc_grp in options['v_region_reports']]
            domain.append(('regiones_id', 'in', v_region_report_ids))
        return domain
    ...
    def _get_lines(self, options, line_id=None):
        ...
        if options.get('v_region_report'):
            if not domain:
                domain = []
            domain += self._get_options_v_region_report_domain(options)
